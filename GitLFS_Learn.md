...
### 1. **节省存储空间**
Git LFS 通过将大文件存储在远程服务器上，而不是直接存储在本地仓库中，从而显著减少本地存储的占用。例如，一个包含多个大型模型文件的机器学习项目，使用 Git LFS 后，本地仓库只需存储指向这些模型文件的指针，而不需要存储完整的模型文件。

### 2. **提高下载效率**
Git LFS 支持按需下载大文件，这意味着在克隆或拉取仓库时，只会下载当前需要的文件，而不是所有文件的历史版本。例如，在游戏开发中，团队成员可以根据需要下载特定的纹理或音频文件，而无需下载整个项目的大型资源。

### 3. **提升性能**
Git LFS 优化了克隆和拉取操作的性能。由于大文件存储在外部服务器上，Git 只需处理小的指针文件，这使得操作速度更快。例如，一个包含多个大型视频文件的多媒体项目，使用 Git LFS 后，克隆速度显著提升。

### 4. **支持团队协作**
Git LFS 使得团队成员可以高效地共享和管理大文件。大文件存储在远程服务器上，所有协作者都可以按需下载，而不会拖慢仓库操作。例如，在一个多人协作的 AI 项目中，团队成员可以轻松共享模型文件，而无需担心存储和带宽问题。

### 5. **版本控制更高效**
Git LFS 提供了对大文件的版本控制功能，同时避免了传统 Git 在处理二进制文件时的性能问题。例如，一个包含多个大型图像文件的设计项目，使用 Git LFS 后，可以轻松管理这些文件的历史版本。

### 实际例子
假设你正在开发一个游戏项目，其中包含大量的纹理文件和音频文件。如果没有 Git LFS，每次克隆或拉取仓库时，都需要下载所有文件的历史版本，这会导致仓库体积膨胀并拖慢操作速度。而使用 Git LFS 后，这些大文件会被存储在远程服务器上，本地仓库只保留指针文件。当需要特定文件时，Git LFS 会按需下载，从而显著提高效率。

总之，Git LFS 是一个强大的工具，特别适合需要管理大文件的项目，如游戏开发、机器学习和多媒体设计等。通过使用 Git LFS，可以显著提升性能、节省存储空间，并优化团队协作。

---

## Git LFS 安装和使用

Git LFS（Large File Storage）是一个Git扩展，用于管理大文件和二进制文件。它通过将大文件存储在外部服务器上，而不是直接存储在Git仓库中，从而优化了仓库的性能。Git LFS的主要功能包括：

1. **管理大文件**：Git LFS允许开发者高效地管理大于普通文件大小（通常大于50MB）的文件，如图像、模型、视频等。
2. **提高性能**：通过将大文件存储在外部系统，Git只会记录文件的元数据，减少了版本控制系统的负担，避免仓库膨胀。
3. **节省存储空间**：通过减少本地仓库中存储大文件的数量，Git LFS帮助节省了硬盘空间，并提高了文件下载速度。

以下是使用Git LFS的详细步骤：

### 安装Git LFS
1. **下载并安装Git LFS**：
   - 访问Git LFS官网下载对应系统的安装包。
   - 或使用包管理器安装（例如macOS上通过Homebrew）：
     ```bash
...
### 配置Git LFS
1. **指定要追踪的大文件类型**：
   - 例如追踪所有`.psd`文件和`data/`目录下的`.zip`文件：
     ```bash
     git lfs track "*.psd"
     git lfs track "data/*.zip"
     ```
2. **提交`.gitattributes`文件**：
   - Git LFS的追踪规则会保存在`.gitattributes`中，需提交此文件到仓库：
     ```bash
     git add .gitattributes
     git commit -m "启用Git LFS追踪大文件"
     ```
...
2. **推送到远程仓库**：
   ...
   ```
   - Git LFS会自动将大文件上传到LFS服务器，并在Git仓库中保留轻量级指针。
...
### 处理已提交到Git历史中的大文件
如果大文件已经存在于Git历史记录中，需清理历史：
1. **使用`git lfs migrate`迁移历史文件**：
   ```bash
   git lfs migrate import --include="*.psd" --everything
   ```
   - `--include`指定要迁移的文件类型，`--everything`处理所有分支和标签。
2. **强制推送清理后的仓库**：
   ```bash
...
通过以上步骤，你可以高效地管理大文件，避免Git仓库的膨胀，同时提高版本控制的效率。





## 参考  Atlassian 官方介绍 Git LFS 的文章 如下
